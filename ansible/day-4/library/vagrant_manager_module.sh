...
function vagrant_up
    {
    status=$(vagrant status | grep kamyshev0 | awk '{print $2}')
...
{   status=$(vagrant status | grep kamyshev0 | head -n1 | awk '{print $2}')
    if [ "$status" == "running" ] || [ "$status" == "poweroff" ]; then
        vagrant destroy --force  &>/dev/null
        status=$(vagrant status | grep kamyshev0 | head -n1 | awk '{print $2 " " $3}')
